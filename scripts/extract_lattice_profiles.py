...
DEFAULT_ENDPOINT = "https://api.sandbox.lattice-data.org"
...
def summaries_to_json(summaries: list[ProfileSummary]) -> str:
    payload = {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--access-key",
        default=os.environ.get("LATTICE_ACCESS_KEY"),
        help="Portal access key (or set LATTICE_ACCESS_KEY)",
    )
    parser.add_argument(
        "--secret-key",
        default=os.environ.get("LATTICE_SECRET_KEY"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = summaries_to_json(summaries)
    if args.include_raw:
